...
def func(VS, IS, N, RS, RP, VG, T=300):
    ...
    w_arg = (IS * RS /(N * VT)) * np.exp((V_adjusted + IS * RS) / (N * VT))
    w = lambertw(w_arg).real
    ...
    # Convert to mA and return linear current
    current_mA = I_total * 1000
    # Use a small positive floor, e.g., 1pA = 1e-9 mA
    # (IS is in Amps, so IS*1000 is saturation current in mA, could be 1e-17 mA or smaller)
    safe_current_mA = np.maximum(current_mA, 1e-9) 
    return safe_current_mA
...
def main():
    ...
    parser.add_argument('-VG', '--VG', type=float, default=1.7, help='Initial guess at bandgap voltage (default = 1.7V)')
...
#Set up initial guess
    # params = dict(IS = args.IS, N = 2.0, RS = 1.0, RP = args.RP, VG = args.VG)
    # Reverting to use command-line/default initial guesses for N and RS
...
#Perform non-linear least squares fit
        try:
            ...
            # IS, N, RS, RP, VG
            # Adjusted lower bounds for N and RS:
            # bounds_lower = [1e-25, 1.5, 0.1, 1e3, 1.0]
            # Reverting N and RS lower bounds to previous values
            bounds_lower = [1e-25, 1.0, 0.01, 1e3, 1.0]
            bounds_upper = [1e-9,  8.0,  500, 1e9, 4.5]

            popt, pcov, infodict, errmsg, ier = curve_fit(
                lambda x, IS, N, RS, RP, VG: func(x, IS, N, RS, RP, VG, args.T), 
                xdata, ydata,  # Changed from logydata to ydata
                p0=(params['IS'], params['N'], params['RS'], params['RP'], params['VG']), 
                bounds=(bounds_lower, bounds_upper),  # Add bounds here
                maxfev=args.maxit, 
                full_output=True)
